...
public class SubscriptionService {
    ...
    public SubscriptionResponse subscribe(CreateSubscriptionRequest request) {
        ...
        boolean isFreePlan = Boolean.TRUE.equals(plan.getIsFree());

        if (isFreePlan) {
            if (request.couponCode() != null && !request.couponCode().isBlank()) {
                throw new BusinessException("Cupom nao pode ser aplicado em assinatura de plano gratuito.");
            }
            if (request.creditCard() != null || request.creditCardToken() != null) {
                throw new BusinessException("Plano gratuito nao aceita dados de cartao de credito.");
            }
            if (request.nextDueDate() != null) {
                throw new BusinessException("Plano gratuito nao permite definir nextDueDate.");
            }
        }
        ...
        if (!isFreePlan && request.couponCode() != null && !request.couponCode().isBlank()) {
        ...
        if (!isFreePlan) {
            ...
            asaasSubscriptionId = asaasResult.asaasId();
        }
        ...
        // Sync charges generated by Asaas for this subscription (skip for free plans)
        if (!isFreePlan) {
